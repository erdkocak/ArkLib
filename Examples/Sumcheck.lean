...
@[simp]
lemma monad_pure_def (x : α) : (pure x : OracleComp spec α) = OracleComp.pure x := rfl

@[simp]
lemma monad_bind_def (x : OracleComp spec α) (f : α → OracleComp spec β) :
    x >>= f = OracleComp.bind x f := rfl
...
@[simp]
def mapLift (pSpec : ProtocolSpec)
    {Output : Type} :
...
variable {mP mV : Type → Type} {m : Type → Type}
  [Monad mP] [Monad mV] [Monad m] [MonadLiftT mP m] [MonadLiftT mV m]
  {InputStatement InputWitness : Type}
  {OutputStatement OutputWitness : Type}
  {pSpec : ProtocolSpec}
...
def run (inputStatement : InputStatement) (inputWitness : InputWitness)
  ...
  let liftedProverProtocol : InteractiveProtocol m pSpec (OutputStatement × OutputWitness) :=
    InteractiveProtocol.mapLift pSpec initialProverProtocol
  let liftedVerifierProtocol : InteractiveProtocol m pSpec.flip OutputStatement :=
    InteractiveProtocol.mapLift pSpec.flip initialVerifierProtocol
...
/-- Input relation: p(0,0) + p(0,1) + p(1,0) + p(1,1) = t -/
def inputRelation : InputStatement F d × InputWitness → Prop :=
...
/-- Output relation: p(r₁, r₂) = t₂ -/
def outputRelation : OutputStatement F d × OutputWitness → Prop :=
  fun ⟨⟨⟨p, _⟩, r₁, r₂, t₂⟩, _⟩ => p.evalXY r₁ r₂ = t₂

/-- Prover strategy: compute s₁ = p(X,0) + p(X,1), then s₂ = p(r₁,X) -/
noncomputable def prover : Prover Id
...
noncomputable def verifier : Verifier (OptionT (OracleComp (fieldSamplingOracle F + bivariatePolyOracle F)))
...
noncomputable def reduction : Reduction
    Id (OptionT (OracleComp (fieldSamplingOracle F + bivariatePolyOracle F)))
    (InputStatement F d) InputWitness (OutputStatement F d) OutputWitness (pSpec F) where
  prover := prover F d
  verifier := verifier F d

-- TODO: define and prove completeness and soundness

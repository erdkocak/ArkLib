...
-- TODO: generic index κ instead of (Fin k)??? Would that be better?
...
/-- Let `u := {u_1, ..., u_l}` be a collection of vectors with coefficients in a semiring.
  ...
  form `{∑ i ∈ k r ^ i • u_i | r ∈ F}`. -/
def parametrisedCurve {k : ℕ} (u : Fin k → ι → A) : Set (ι → A)
...
instance [Nonempty F] {k : ℕ} :
  ∀ u : Fin k → ι → A, Nonempty {x // x ∈ parametrisedCurveFinite (F := F) u } := by
  intro u
...
open Finset
instance {k : ℕ} {u : Fin k → ι → A} : Nonempty {x // x ∈ parametrisedCurveFinite (F := F) u} := by

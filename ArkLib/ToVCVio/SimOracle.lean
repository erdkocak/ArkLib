...
DT: Everything here is be ported or subsumed by newer things at this point.
...
-- def simulateQ' (oa : OracleComp spec α) (h : HasEvalSPMF.NeverFail oa) : m α := by
...
-- @[reducible]
-- def SimOracle.Stateful (spec specₜ : OracleSpec) (σ : Type) :=
--   QueryImpl spec (StateT σ (OracleComp specₜ))

-- @[reducible]
-- def SimOracle.Stateless (spec specₜ : OracleSpec) :=
--   QueryImpl spec (OracleComp specₜ)

-- @[reducible]
-- def SimOracle.Impl (spec : OracleSpec) := SimOracle.Stateless spec []ₒ
...
-- def dedup {ι : Type} (spec : OracleSpec ι) : SimOracle.Stateless (spec + spec) spec where
--   impl
--   | query (.inl i) t => query i t

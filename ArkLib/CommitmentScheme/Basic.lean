...
variable (O : OracleContext Unit (ReaderM Data)) {n : ℕ} (pSpec : ProtocolSpec n)

structure Opening where
  opening : Proof oSpec (Commitment × O.spec.Domain × O.spec.Range ()) (Data × Randomness) pSpec
...
structure Scheme extends
    ...
    Opening oSpec Data Randomness Commitment O pSpec
...
variable [∀ i, VCVCompatible (pSpec.Challenge i)] [DecidableEq ι]
  {oSpec : OracleSpec ι} {Data : Type} (O : OracleContext Unit (ReaderM Data)) {Randomness : Type}
  [Fintype Randomness]
  {Commitment : Type} {n : ℕ} {pSpec : ProtocolSpec n}
...
def correctness (scheme : Scheme oSpec Data Randomness Commitment O pSpec)
...
-/
def perfectCorrectness (scheme : Scheme oSpec Data Randomness Commitment O pSpec) : Prop :=
  correctness O scheme 0
...
def binding (scheme : Scheme oSpec Data Randomness Commitment O pSpec)
    (bindingError : ℝ≥0) : Prop :=
  ∀ AuxState : Type,
  ∀ adversary : BindingAdversary O oSpec Data Commitment AuxState,
  ∀ prover : Prover oSpec (Commitment × O.spec.Domain × O.spec.Range ()) AuxState Bool Unit pSpec,
    False

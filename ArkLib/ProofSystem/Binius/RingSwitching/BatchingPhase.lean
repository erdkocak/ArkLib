...
2. `V` decomposes `ŝ =: Σ_{v ∈ B_κ} ŝ_v ⊗ β_v`.
  `V` requires `s ?= Σ_{v ∈ B_κ} eq̃(v_0, ..., v_{κ-1}, r_0, ..., r_{κ-1}) ⋅ ŝ_v`.
...
4. For each `w ∈ B_{ℓ'}`,
  ...
  `P` defines the function
    ...
    on `B_{ℓ'}` and writes `A(X_0, ..., X_{ℓ'-1})` for its multilinear extension.
  `P` defines `h(X_0, ..., X_{ℓ'-1}) := A(X_0, ..., X_{ℓ'-1}) ⋅ t'(X_0, ..., X_{ℓ'-1})`.c
5. `V` decomposes `ŝ =: Σ_{u ∈ B_κ} β_u ⊗ ŝ_u`, and
  sets `s_0 := Σ_{u ∈ B_κ} eq̃(u_0, ..., u_{κ-1}, r''_0, ..., r''_{κ-1}) ⋅ ŝ_u`.
...
theorem batchingOracleVerifier_rbrKnowledgeSoundness :
  ...
  -- `KState 1 = (t' = packMLE t) ∧ (ŝ := φ₁(t')(φ₀(r_κ), ..., φ₀(r_{ℓ-1})))`
    -- `∧ (s ?= Σ_{v ∈ B_κ} eqTilde(v, r_{0..κ-1}) ⋅ ŝ_v.`
  -- `KState 2 = (s ?= Σ_{v ∈ B_κ} eqTilde(v, r_{0..κ-1}) ⋅ ŝ_v) ∧`
    -- `h = projectSumcheckPoly t' 0 r r' ∧ s_0 = Σ_{w ∈ B_{ℓ'}} h(w)`

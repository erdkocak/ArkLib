...
open Module
/-- Decompose `ŝ` into row components `(ŝ =: Σ_{u ∈ B_κ} β_u ⊗ ŝ_u)`.
...
/-- Decompose `ŝ` into column components `(ŝ =: Σ_{v ∈ B_κ} ŝ_v ⊗ β_v)`.
...
For each `w ∈ B_ℓ'`, the evaluation `t'(w)` is defined as:
`t'(w) := ∑_{v ∈ B_κ} t(v₀, ..., v_{κ-1}, w₀, ..., w_{ℓ'-1}) ⋅ β_v`
-/
...
/-- Step 2 (V): Check 1: s ?= Σ_{v ∈ B_κ} eqTilde(v, r_{0..κ-1}) ⋅ ŝ_v. -/
...
/-- Step 4a: For each `w ∈ B_{ℓ'}`, P decompose `eq̃(r_κ, ..., r_{ℓ-1}, w_0, ..., w_{ℓ'-1})`
`=: Σ_{u ∈ B_κ} A_{w, u} ⋅ β_u`.
P define the function
`A: w ↦ Σ_{u ∈ B_κ} eq̃(u_0, ..., u_{κ-1}, r''_0, ..., r''_{κ-1}) ⋅ A_{w, u}`
on `B_{ℓ'}`.
-/
def compute_A_func (original_r_eval_suffix : Fin ℓ' → L)
  ...
  fun w =>
    ...
    -- Compute A(w) = Σ_{u ∈ B_κ} eq̃(u, r'') ⋅ A_{w,u}
...
/-- Step 5 (V): Compute `s₀ := Σ_{u ∈ B_κ} eqTilde(u, r'') ⋅ ŝ_u`,
...
/-- Decompose the final eq tensor `e := Σ_{u ∈ B_κ} eq̃(u, r'') ⨂ e_u`,
where e_u is the row components of e.
Then compute `Σ_{u ∈ B_κ} eq̃(u_0, ..., u_{κ-1}, r''_0, ..., r''_{κ-1}) ⋅ e_u`.
-/
